    ...
    switch (kpiType) {
      ...
      case "fullYearForecast":
        ...
        if (typeof value === "number") {
          ...
          if (annualTarget > 0) {
            const forecastVsTarget = ((value - annualTarget) / annualTarget) * 100;
            ...
            <div className="kpi-row">
              <div className="kpi-cards">
                ...
                <KPICard
                  ...
                  isPercentage={true}
                  subtitle={getTargetAchievementSubtitle(kpis.targetAchievement)}
                  kpiType={getPerformanceClass(
                    "targetAchievement",
        ...
        {!monthlyTrendTableCollapsed && (
          <div className="trend-data-table">
            <table className="trend-table monthly-trend-table">
              ...
              <tbody>
                {trend.map((data, index) => (
                  <tr key={index} className={!data.isFinalMonth ? "table-row-forecast" : ""}>
                    <td>{data.period}</td>
                    <td>
            ...
            return (
              <>
                ...
                <div className="kpi-card">
                  ...
                  <strong>{formatCurrencyFull(avgQuarterlyAdjusted)}</strong>
                  <div className="kpi-description-text">
        ...
        >
          ...
          {totalCompCapitalizationCollapsed && (
            <div className="compact-summary">
              ...
              <span className="compact-metric">
                YTD: <strong>{formatCurrencyFull(resourceData.totalCompensation.ytdActual)}</strong>
              </span>
              <span className="compact-metric">
                Budget: <strong>{formatCurrencyFull(resourceData.totalCompensation.annualBudget)}</strong>
              </span>
              <span className="compact-metric">
                Remaining: <strong>{formatCurrencyFull(resourceData.totalCompensation.remaining)}</strong>
              </span>
            </div>
          )}
        </div>
