...
import { TableActionButtons } from "../../../components/shared";
...
const VendorBudgetTable: React.FC<VendorBudgetTableProps> = ({
  ...
  // Get filtered and sorted vendors - memoized for performance
  ...
  const totalBudget = useMemo(() => {
    return filteredAndSortedVendors.reduce(
      (sum, vendor) => sum + vendor.budget,
      0
    );
  }, [filteredAndSortedVendors]);

  const inBudgetTotal = useMemo(() => {
    return filteredAndSortedVendors
      .filter((vendor) => vendor.inBudget)
      .reduce((sum, vendor) => sum + vendor.budget, 0);
  }, [filteredAndSortedVendors]);

  const notInBudgetTotal = useMemo(() => {
    return filteredAndSortedVendors
      .filter((vendor) => !vendor.inBudget)
      .reduce((sum, vendor) => sum + vendor.budget, 0);
  }, [filteredAndSortedVendors]);
          ...
          <tbody>
            ...
            <tr className="totals-row">
              ...
              <td className="totals-amount">
                <strong>
                  {formatCurrencyExcelStyle(inBudgetTotal * 1000)}
                </strong>
              </td>
              <td colSpan={5}></td>
            </tr>
...
export default React.memo(VendorBudgetTable);
